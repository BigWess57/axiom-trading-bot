...
class StrategyRandomizer:
    ...
    @staticmethod
    def generate_randomized_configs(num_configs: int, strategy_type: str = "core") -> Dict[str, Dict[str, Any]]:
        """
        ...
        for i in range(num_configs):
            if strategy_type == "baseline":
                ...
                conf = get_baseline_config()
                # --- Baseline Randomized Parameters ---
                conf["min_holder_sol_balance"] = round(random.uniform(0.5, 2.0), 1)
                conf["holder_check_count"] = random.choice([20, 25, 30])
                conf["max_top10_percent"] = round(random.uniform(40.0, 60.0), 1)
                conf["max_bundled_percent"] = round(random.uniform(40.0, 60.0), 1)
                conf["min_pro_trader_percent"] = round(random.uniform(20.0, 30.0), 1)
                
                conf["min_market_cap"] = round(random.uniform(5500, 8000), 2)
                conf["max_market_cap"] = round(random.uniform(17000, 20000), 2)
                
                conf["min_txns_per_min"] = random.randint(20, 80)
                conf["min_buy_sell_ratio"] = round(random.uniform(1.5, 3.0), 1)
                conf["min_users_watching_increase"] = random.randint(10, 100)
                conf["activity_lookback_seconds"] = random.choice([30, 45, 60, 90, 120])
                
                conf["initial_stop_loss_pct"] = round(random.uniform(0.20, 0.40), 2)
                conf["max_take_profit_pct"] = round(random.uniform(0.80, 2.00), 2)
                conf["max_holding_time"] = random.randint(90, 600)
            ...
            bot_id = f"{strategy_type.upper()}_Bot_{i:03d}"
