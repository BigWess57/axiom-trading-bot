...
class BaselineSecurityMixin:
    """Mixin for baseline strategy security checks"""
    ...
    def _security_checkup(self, token: PulseToken, sol_price: float, holder_safety_score: float) -> Optional[str]:
        """
        Perform strict security checkups.
        Returns None if all checks pass.
        If any checks fail, returns a string describing the failure reason.
        """
        ...
        if token.fees_paid > 0 and token.volume_total * sol_price / token.fees_paid > self.config.safety.max_volume_fees_ratio:
            ...
            return f"Volume/fees ratio too high ({ratio:.0f}, max {self.config.safety.max_volume_fees_ratio})"
        if holder_safety_score < self.config.safety.holder_safe_threshold:
            return f"Holder safety score is too low ({holder_safety_score:.2f}, min {self.config.safety.holder_safe_threshold})"
